...
func (s *oldDefaultSelector) AllPaths() []*pan.Path {
	s.mutex.Lock()
...
func (s *oldDefaultSelector) Path() *pan.Path {
	return s.current
}

func (s *oldDefaultSelector) setCurrent(path *pan.Path) {
	s.current = path
	if path == nil {
		s.pathFixed = false
	}
...
func (s *oldDefaultSelector) SetPaths(paths []*pan.Path) {
	if s.observer != nil {
...
func (s *oldDefaultSelector) OnPathDown(pf pan.PathFingerprint, pi pan.PathInterface) {
